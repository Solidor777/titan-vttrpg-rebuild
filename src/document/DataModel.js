...
export default class TitanDataModel extends foundry.abstract.TypeDataModel {
    ...
    * @type {DocumentDataModelMetadata}
    */
   static metadata = Object.freeze({
      documentFlagsModel: null
   });
   ...
   /**
    * Pre-creation logic for this document data.
    ...
    * @returns {typeof TitanDataModel}  Final prepared type.
    */
   static mixin(...templates) {
      for ( const template of templates ) {
         if ( !(template.prototype instanceof TitanDataModel) ) {
            throw new Error(`${template.name} is not a subclass of DocumentDataModel`);
         }
      }
...
/**
 * Variant of the DocumentDataModel with some extra actor-specific handling.
 */
export class ActorDataModel extends TitanDataModel {

   /**
    * @typedef {DocumentDataModelMetadata} ActorDataModelMetadata
   ...
   get transferDestinations() {
      ...
      if ( !primaryParty?.document.members.ids.has(this.parent.id) ) return [];
      const destinations = primaryParty.document.members.map(m => m.actor).filter(a => a.isOwner && a !== this.parent);
...
/**
 * Variant of the DocumentDataModel with support for rich item tooltips.
 */
export class ItemDataModel extends TitanDataModel {

   /**
    * @typedef {DocumentDataModelMetadata} ItemDataModelMetadata
    ...
    * @type {string}
    */
   static ITEM_TOOLTIP_TEMPLATE = "documents/dnd5e/templates/items/parts/item-tooltip.hbs";
